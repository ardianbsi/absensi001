...
@section('content')
    <x-breadcrumb :items="[['name' => 'Employees', 'route' => 'employees.index'], ['name' => $employee->full_name, 'route' => 'employees.show', 'route_param' => $employee], ['name' => 'Edit']]" />

    <x-page-header title="Edit Employee" subtitle="Update employee information" />
            ...
            <form action="{{ route('employees.update', $employee) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="birth_date" class="form-control datepicker @error('birth_date') is-invalid @enderror" value="{{ old('birth_date', $employee->birth_date?->format('Y-m-d')) }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="position_id" class="form-select tom-select @error('position_id') is-invalid @enderror" required>
                            ...
                            @foreach($positions as $id => $name)
                                <option value="{{ $id }}" {{ old('position_id', $employee->position_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="join_date" class="form-control datepicker @error('join_date') is-invalid @enderror" value="{{ old('join_date', $employee->join_date?->format('Y-m-d')) }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="manager_id" class="form-select tom-select @error('manager_id') is-invalid @enderror">
                            ...
                            @foreach($managers as $id => $name)
                                <option value="{{ $id }}" {{ old('manager_id', $employee->manager_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <small class="form-hint">Leave empty to keep current photo. Max 2MB.</small>
                        @error('photo') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <img id="photo-preview" src="{{ $employee->photo ? asset('storage/' . $employee->photo) : '#' }}" alt="Preview" class="{{ $employee->photo ? '' : 'd-none' }}" style="max-height: 120px; border-radius: 8px;">
                    </div>
                </div>

                <div class="form-check mb-3">
                    <input type="checkbox" name="is_active" class="form-check-input" value="1" id="is_active" {{ old('is_active', $employee->is_active) ? 'checked' : '' }}>
